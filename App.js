...
console.log(Query.fromArray([1, 2, 3]).toList().where(function (n) {
    return n > 1;
}).toList());

console.log(Query.fromArray(['t', 'e', 's', 't']).aggregate(function (c, n) {
    return c + n;
}, ''));

console.log(Query.fromArray(['t', 'e', 's', 't']).skip(1).aggregate(function (c, n) {
    return c + n;
}, ''));
...
var persons = [
    new Person('Thomas', 26, 0 /* Male */),
    new Person('Lasse', 22, 0 /* Male */),
    new Person('Caroline', 24, 1 /* Female */)
];

Query.fromArray(persons).groupBy(function (p) {
    return p.sex;
}).toList().each(function (g) {
    return g.orderByAscending(function (p2) {
        return p2.age;
    }).toList().each(function (p) {
